...
// 					$search_user_id = $rsp->graphql->user->id;
// 					delete_transient( $transient_name );
// 					set_transient( $transient_name, $search_user_id, $this->transient_sec );
// 				}
// 			}

			/*$data = wp_remote_get("https://www.instagram.com/themepunch/");
			...
			$cookie = "";
			foreach ($data["headers"]["set-cookie"] as $set_cookie) {
			$set_cookie = explode(";", $set_cookie);
			$cookie .= $set_cookie[0]."; ";
			if(strpos($set_cookie[0], "csrftoken")!==false)
				$csrftoken = str_replace("csrftoken=", "", $set_cookie[0]);
				}
				...
				//"Cookie: rur=FRC; csrftoken=t2n67am9v0OLot0Fr89EXCvAxGsGYakq; mid=WstT0wALAAHEdp3EJcnGPbTr9qBa; urlgen=\"{\"time\": 1523274707\054 \"188.93.10.98\": 20773}:1f5VKu:6rZPGEOhyKQTLZr2quAaYB6Wsw4\"; ig_or=landscape-primary; ig_pr=1; ig_vh=887; ig_vw=1660"
				"Cookie: ".$cookie." ig_or=landscape-primary; ig_pr=1; ig_vh=".$ig_vh."; ig_vw=".$ig_vw.""
				);
				*/
			...
			if ($this->transient_sec > 0 && false !== ($data = get_transient( $transient_name))){
				...
				//if(isset($_GET['dg'])){echo '<pre>';   print_r($rsp);   echo '</pre>';}
...
// 					if(!isset($rsp->edge_owner_to_timeline_media) || !$rsp->edge_owner_to_timeline_media->count){
// 						_e('<p>Instagram reports: Please check the settings</p>',EG_TEXTDOMAIN);
// 						return false;
// 					}
					/*while($count){
					 ...
					 $count = $this->instagram_output_array($rsp->graphql->user->edge_owner_to_timeline_media->edges,$count,$search_user_id,$orig_image);
					 }*/
	...
	public function getFeedByID($url,$args) {

		$ig_vh = rand(500, 1000);
		$ig_vw = rand(1100, 2000);

		$curl_server = function_exists('curl_init');
		$sockets_server = function_exists('fsockopen');
		if($curl_server){
			$curl1 = curl_init();
			$curl_options = array(
					CURLOPT_RETURNTRANSFER => true,
					CURLOPT_HEADER => true,
					CURLOPT_URL => "https://www.instagram.com/themepunch/",
			...
			curl_setopt_array($curl1, $curl_options);

			$response_str = curl_exec($curl1);
			$curl_info = curl_getinfo($curl1);
			$curl_error = curl_error($curl1);

			curl_close($curl1);

			if(isset($_GET['dg'])){echo '<pre>';   var_dump($response_str);   echo '</pre>';}
			...
			$arr = explode("\n", $response_array[0]);
			foreach($arr as $a){
				if(strpos($a, "Set-Cookie:") !== false){
					$as = explode(';', $a);
					foreach($as as $aa){
						if(trim($aa) === "") continue;
						if(strpos($aa, "expires") !== false) continue;
						if(strpos($aa, "Max-Age") !== false) continue;
						if(strpos($aa, "Path=/") !== false) continue;
						//if(strpos($aa, "Secure") !== false) continue;

						$new = trim(str_replace(array("Set-Cookie: ", "Set-Cookie:", "Path=/", "Secure"), "", $aa));
						$new = str_replace("; ;", "", $new);
						$new = str_replace(";", "", $new);

						if(empty(trim($new))) continue;
						//echo '|'.$new.'|<br>';
						if($new !== ';') $cookie.= $new.'; ';
					}
				}
				if(strpos($a, "csrftoken=") !== false){
					$csrftoken = explode(';', $a);
					$csrftoken = str_replace(array("csrftoken=", "Set-Cookie: ", "Set-Cookie:"), "", $csrftoken[0]);
				}
			}
			/*echo '<pre>';
			 print_r($arr);
			 ...
			 foreach ($data["headers"]["set-cookie"] as $set_cookie) {
				...
				if(isset($_GET['dg'])){echo '<pre>';   var_dump($curl_options);   echo '</pre>';}
	...
	public function get_place_photos($search_user_id,$count,$orig_image){
		if(!empty($search_user_id)){
			...
			if ($this->transient_sec > 0 && false !== ($data = get_transient( $transient_name))){
				...
				$count = $this->instagram_output_array_places($rsp->location->media->nodes,$count,$search_user_id,$orig_image);
