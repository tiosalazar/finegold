...
if ( !function_exists( 'planmyday_sc_form_send' ) ) {
	function planmyday_sc_form_send() {
		...
		if (!($contact_email = planmyday_get_theme_option('contact_email')) && !($contact_email = planmyday_get_theme_option('admin_email'))) 
			$response['error'] = esc_html__('Unknown admin email!', 'planmyday');
		else {
			...
			if (in_array($type, array('form_1', 'form_2'))) {
				...
				$subj = sprintf(esc_html__('Site %s - Contact form message from %s', 'planmyday'), get_bloginfo('site_name'), $user_name);
				$msg = "\n".esc_html__('Name:', 'planmyday')   .' '.esc_html($user_name)
					.  "\n".esc_html__('E-mail:', 'planmyday') .' '.esc_html($user_email)
					.  "\n".esc_html__('Subject:', 'planmyday').' '.esc_html($user_subj)
					.  "\n".esc_html__('Message:', 'planmyday').' '.esc_html($user_msg);

			} else {

				$subj = sprintf(esc_html__('Site %s - Custom form data', 'planmyday'), get_bloginfo('site_name'));
				$msg = '';
            ...
            if (is_email($contact_email) && !@$mail($contact_email, $subj, apply_filters('planmyday_filter_form_send_message', $msg))) {
				$response['error'] = esc_html__('Error send message!', 'planmyday');
			}
