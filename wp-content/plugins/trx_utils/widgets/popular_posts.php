...
// Theme init
if (!function_exists('planmyday_widget_popular_posts_theme_setup')) {
	add_action( 'planmyday_action_before_init_theme', 'planmyday_widget_popular_posts_theme_setup', 1 );
	function planmyday_widget_popular_posts_theme_setup() {

		// Register shortcodes in the shortcodes list
		if (function_exists('planmyday_exists_visual_composer') && planmyday_exists_visual_composer())
			add_action('planmyday_action_shortcodes_list_vc','planmyday_widget_popular_posts_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('planmyday_widget_popular_posts_load')) {
	add_action( 'widgets_init', 'planmyday_widget_popular_posts_load' );
	function planmyday_widget_popular_posts_load() {
		register_widget('planmyday_widget_popular_posts');
	}
}

// Widget Class
class planmyday_widget_popular_posts extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_popular_posts', 'description' => esc_html__('The most popular and most commented blog posts (extended)', 'planmyday'));
		parent::__construct( 'planmyday_widget_popular_posts', esc_html__('Planmyday - Most Popular & Commented Posts', 'planmyday'), $widget_ops );
	...
	function widget($args, $instance) {
		...
		$show_counters = isset($instance['show_counters']) && $instance['show_counters'] > 0 ? planmyday_get_theme_option('blog_counters') : '';
		...
		for ($i=0; $i<3; $i++) {
			...
			if ($i==0) {			// Most popular
				$args['meta_key'] = planmyday_storage_get('options_prefix').'_post_views_count';
				$args['orderby'] = 'meta_value_num';
				$show_counters = $show_counters ? 'views' : '';
			} else if ($i==2) {		// Most liked
				$args['meta_key'] = planmyday_storage_get('options_prefix').'_post_likes_count';
				$args['orderby'] = 'meta_value_num';
	...
	function form($instance) {
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title_popular')); ?>"><?php esc_html_e('Most popular tab title:', 'planmyday'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title_commented')); ?>"><?php esc_html_e('Most commented tab title:', 'planmyday'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title_liked')); ?>"><?php esc_html_e('Most liked tab title:', 'planmyday'); ?></label>
...
*/
if ( !function_exists( 'planmyday_sc_widget_popular_posts' ) ) {
	function planmyday_sc_widget_popular_posts($atts, $content=null){	
		...
		extract($atts);
		$type = 'planmyday_widget_popular_posts';
		$output = '';
		global $wp_widget_factory;
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_popular_posts' 
								. (planmyday_exists_visual_composer() ? ' vc_widget_popular_posts wpb_content_element' : '') 
								. (!empty($class) ? ' ' . esc_attr($class) : '') 
						. '">';
			ob_start();
			the_widget( $type, $atts, planmyday_prepare_widgets_args(planmyday_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_popular_posts', 'widget_popular_posts') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('planmyday_shortcode_output', $output, 'trx_widget_popular_posts', $atts, $content);
	}
	planmyday_require_shortcode("trx_widget_popular_posts", "planmyday_sc_widget_popular_posts");
}


// Add [trx_widget_popular_posts] in the VC shortcodes list
if (!function_exists('planmyday_widget_popular_posts_reg_shortcodes_vc')) {
	//add_action('planmyday_action_shortcodes_list_vc','planmyday_widget_popular_posts_reg_shortcodes_vc');
	function planmyday_widget_popular_posts_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Widget Popular Posts", 'planmyday'),
				"description" => wp_kses_data( __("Insert popular posts list with thumbs and post's meta", 'planmyday') ),
				...
				"params" => array(
					...
					array(
						...
						"heading" => esc_html__("Most popular tab title", 'planmyday'),
						"description" => wp_kses_data( __("Most popular tab title", 'planmyday') ),
						"class" => "",
						"value" => "",
						"type" => "textfield"
					),
					array(
						...
						"heading" => esc_html__("Most commented tab title", 'planmyday'),
						"description" => wp_kses_data( __("Most commented tab title", 'planmyday') ),
						"class" => "",
						"value" => "",
						"type" => "textfield"
					),
					array(
						...
						"heading" => esc_html__("Most liked tab title", 'planmyday'),
						"description" => wp_kses_data( __("Most liked tab title", 'planmyday') ),
						"class" => "",
						"value" => "",
						"type" => "textfield"
					),
