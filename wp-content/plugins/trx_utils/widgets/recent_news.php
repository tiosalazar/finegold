...
// Theme init
if (!function_exists('planmyday_widget_recent_news_theme_setup')) {
	add_action( 'planmyday_action_before_init_theme', 'planmyday_widget_recent_news_theme_setup', 1 );
	function planmyday_widget_recent_news_theme_setup() {
...
// Load widget
if (!function_exists('planmyday_widget_recent_news_load')) {
	add_action( 'widgets_init', 'planmyday_widget_recent_news_load' );
	function planmyday_widget_recent_news_load() {
		register_widget('planmyday_widget_recent_news');
	}
}


// Widget Class
class planmyday_widget_recent_news extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_recent_news', 'description' => esc_html__('Show recent news in many styles', 'planmyday'));
		parent::__construct( 'planmyday_widget_recent_news', esc_html__('Planmyday - Recent News', 'planmyday'), $widget_ops );
	}
	...
	function widget($args, $instance) {
		...
		$output = planmyday_sc_recent_news( array(
			'from_widget'		=> true,
		...
		if (!empty($output)) {
			...
			if ($widget_title) planmyday_show_layout($title, $before_title, $after_title);
	...
	function form($instance) {
		...
		$news_styles = planmyday_get_list_templates('news');
		?>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id('widget_title')); ?>"><?php esc_html_e('Widget title:', 'planmyday'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e('Block title:', 'planmyday'); ?></label>
			<input id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" value="<?php echo esc_attr($title); ?>" class="widgets_param_fullwidth" />
		</p>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id('subtitle')); ?>"><?php esc_html_e('Block subtitle:', 'planmyday'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('style')); ?>"><?php esc_html_e('Style:', 'planmyday'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('count')); ?>"><?php esc_html_e('Number of posts to be displayed:', 'planmyday'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('featured')); ?>"><?php esc_html_e('Number of featured posts:', 'planmyday'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('columns')); ?>"><?php esc_html_e('Number of columns:', 'planmyday'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_categories')); ?>_1"><?php esc_html_e('Show categories:', 'planmyday'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_categories')); ?>_1"><?php esc_html_e('Show', 'planmyday'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_categories')); ?>_0"><?php esc_html_e('Hide', 'planmyday'); ?></label>
		</p>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_counters')); ?>_1"><?php esc_html_e('Show counters:', 'planmyday'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_counters')); ?>_1"><?php esc_html_e('Show', 'planmyday'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_counters')); ?>_0"><?php esc_html_e('Hide', 'planmyday'); ?></label>
		</p>
...
*/
if ( !function_exists( 'planmyday_sc_recent_news' ) ) {
	function planmyday_sc_recent_news($atts, $content=null){	
		...
		// Get categories list
		if (planmyday_param_is_on($show_categories)) {
			if ( !planmyday_storage_isset('categories_'.$category) ) {
				planmyday_storage_set('categories_'.$category, get_categories(array(
					'orderby' => 'name',
					'parent' => $category
					))
				);
		...
		if (empty($from_widget)) {
			$widget_args = planmyday_prepare_widgets_args(planmyday_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_recent_news', 'widget_recent_news');
			$output .= '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_recent_news_wrap' 
								. (planmyday_exists_visual_composer() ? ' vc_recent_news wpb_content_element' : '') 
		...
		// Header
		if ( !empty($title) || !empty($subtitle) || (planmyday_param_is_on($show_categories) && !planmyday_storage_empty('categories_'.$category)) ) {
			$output	.= '<div class="sc_recent_news_header'.(planmyday_param_is_on($show_categories) && !planmyday_storage_empty('categories_'.$category) ? ' sc_recent_news_header_split' : '').'">'
			...
			// Categories list
			$cat_list = planmyday_storage_get('categories_'.$category);
			if (planmyday_param_is_on($show_categories) && !empty($cat_list)) {
				...
				if (is_array($cat_list) && count($cat_list) > 0) {
					$output .= '<a href="' . esc_url( $category == 0 
						...
						: get_category_link($category) ) . '" class="sc_recent_news_header_category_item">'.esc_html__('All News', 'planmyday').'</span>';
					$number = 0;
					$number_max = 3;
					foreach ($cat_list as $cat) {
						$number++;
						if ($number == $number_max)
							$output .= '<span class="sc_recent_news_header_category_item sc_recent_news_header_category_item_more">'.esc_html__('More', 'planmyday')
		...
		$args = planmyday_query_add_posts_and_cats($args, $ids, $post_type, $category, $taxonomy);
		...
		return apply_filters('planmyday_shortcode_output', $output, 'trx_recent_news', $atts, $content);
	}
	planmyday_require_shortcode("trx_recent_news", "planmyday_sc_recent_news");
}


// Add [trx_recent_news] in the VC shortcodes list
if (!function_exists('planmyday_widget_recent_news_reg_shortcodes_vc')) {
	//add_action('planmyday_action_shortcodes_list_vc','planmyday_widget_recent_news_reg_shortcodes_vc');
	function planmyday_widget_recent_news_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Recent News", 'planmyday'),
				"description" => wp_kses_data( __("Insert recent news list", 'planmyday') ),
				...
				"params" => array(
					array(
						"param_name" => "style",
						"heading" => esc_html__("List style", 'planmyday'),
						"description" => wp_kses_data( __("Select style to display news list", 'planmyday') ),
						"class" => "",
						"admin_label" => true,
						"std" => 'news-magazine',
					...
					array(
						...
						"description" => wp_kses_data( __("Title for the widget (fill this field only if you want to use shortcode as widget)", 'planmyday') ),
						"admin_label" => true,
						"class" => "",
						"value" => "",
						"type" => "textfield"
					...
					array(
						...
						"heading" => esc_html__("Show categories", 'planmyday'),
						"description" => wp_kses_data( __("Show categories in the shortcode's header", 'planmyday') ),
						"class" => "",
					...
					array(
						...
						"heading" => esc_html__("Show counters", 'planmyday'),
						"description" => wp_kses_data( __("Show counters (comments and views) in the shortcode's footer", 'planmyday') ),
						'dependency' => array(
							'element' => 'style',
							'value' => 'news-magazine'
						),
					...
					array(
						"param_name" => "cat",
						"heading" => esc_html__("Category", 'planmyday'),
						"description" => wp_kses_data( __("Select category to show news. If empty - select news from any category or from IDs list", 'planmyday') ),
						"group" => esc_html__('Query', 'planmyday'),
						"class" => "",
					...
					array(
						"param_name" => "count",
						"heading" => esc_html__("Total posts", 'planmyday'),
					...
					array(
						"param_name" => "featured",
						"heading" => esc_html__("Featured posts", 'planmyday'),
						"description" => wp_kses_data( __("How many posts will be displayed as featured?", 'planmyday') ),
						"admin_label" => true,
						"group" => esc_html__('Query', 'planmyday'),
						'dependency' => array(
							'element' => 'style',
					...
					array(
						...
						"description" => wp_kses_data( __("How many columns use to show news list", 'planmyday') ),
						"group" => esc_html__('Query', 'planmyday'),
						"admin_label" => true,
						'dependency' => array(
							'element' => 'style',
					...
					array(
						...
						"group" => esc_html__('Query', 'planmyday'),
						"class" => "",
						"value" => array_flip(planmyday_get_list_sortings()),
						"type" => "dropdown"
					),
					array(
						"param_name" => "order",
						...
						"group" => esc_html__('Query', 'planmyday'),
						"class" => "",
						"value" => array_flip(planmyday_get_list_orderings()),
						"type" => "dropdown"
					),
					array(
						"param_name" => "ids",
						"heading" => esc_html__("List IDs", 'planmyday'),
						"description" => wp_kses_data( __("Comma separated list of post's ID. If set - parameters above (category, count, order, etc.) are ignored!", 'planmyday') ),
						"group" => esc_html__('Query', 'planmyday'),
						"class" => "",
						"value" => "",
						"type" => "textfield"
