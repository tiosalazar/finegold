...
function planmyday_theme_customizer(option, val) {
	...
	if (option == 'body_style') {
		...
		var params = PLANMYDAY_STORAGE['co_add_params']!=undefined ? PLANMYDAY_STORAGE['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#planmyday-customizer-styles-'+option).length > 0 ? jQuery('#planmyday-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="planmyday-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
