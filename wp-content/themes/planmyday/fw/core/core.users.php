<?php
/**
 * Planmyday Framework: Registered Users
 *
 * @package	planmyday
 * @since	planmyday 1.0
 */
...
// Theme init
if (!function_exists('planmyday_users_theme_setup')) {
	add_action( 'planmyday_action_before_init_theme', 'planmyday_users_theme_setup' );
	function planmyday_users_theme_setup() {
        // Social Login support
        add_filter( 'trx_utils_filter_social_login', 'planmyday_social_login');

		if ( is_admin() ) {
			// Add extra fields in the user profile
			add_action( 'show_user_profile',		'planmyday_add_fields_in_user_profile' );
			add_action( 'edit_user_profile',		'planmyday_add_fields_in_user_profile' );
	
			// Save / update additional fields from profile
			add_action( 'personal_options_update',	'planmyday_save_fields_in_user_profile' );
			add_action( 'edit_user_profile_update',	'planmyday_save_fields_in_user_profile' );
		}

	}
    // Return Social Login layout (if present)
    if (!function_exists('planmyday_social_login')) {
        function planmyday_social_login($sc) {
            return planmyday_get_theme_option('social_login');
        }
    }
}
...
if (!function_exists('planmyday_show_user_socials')) {
	function planmyday_show_user_socials($args) {
		...
		if (count($list) > 0) {
			$output = '<div class="sc_socials sc_socials_size_'.esc_attr($args['size']).' sc_socials_shape_'.esc_attr($args['shape']).' sc_socials_type_' . esc_attr($args['style']) . '">' 
							. trim(planmyday_prepare_socials($list, $args['style'])) 
						. '</div>';
...
if (!function_exists('planmyday_add_fields_in_user_profile')) {
	function planmyday_add_fields_in_user_profile( $user ) { 
	?>
		<h3><?php esc_html_e('User Position', 'planmyday'); ?></h3>
		<table class="form-table">
			<tr>
				<th><label for="user_position"><?php esc_html_e('User position', 'planmyday'); ?>:</label></th>
				<td><input type="text" name="user_position" id="user_position" size="55" value="<?php echo esc_attr(get_the_author_meta('user_position', $user->ID)); ?>" />
					<span class="description"><?php esc_html_e('Please, enter your position in the company', 'planmyday'); ?></span>
				</td>
			</tr>
		</table>
	
		<h3><?php esc_html_e('Social links', 'planmyday'); ?></h3>
		<table class="form-table">
		<?php
		...
		if (is_array($social_list) && count($social_list) > 0) {
			foreach ($social_list as $soc) {
				...
				if (!empty($sn)) {
					?>
					<tr>
						<th><label for="user_<?php echo esc_attr($sn); ?>"><?php planmyday_show_layout(planmyday_strtoproper($sn)); ?>:</label></th>
						<td><input type="text" name="user_<?php echo esc_attr($sn); ?>" id="user_<?php echo esc_attr($sn); ?>" size="55" value="<?php echo esc_attr(get_the_author_meta('user_'.($sn), $user->ID)); ?>" />
							<span class="description"><?php echo sprintf(esc_html__('Please, enter your %s link', 'planmyday'), planmyday_strtoproper($sn)); ?></span>
						</td>
					</tr>
					<?php
				}
...
if (!function_exists('planmyday_save_fields_in_user_profile')) {
	function planmyday_save_fields_in_user_profile( $user_id ) {
