...
if ( !function_exists( 'planmyday_template_header_4_theme_setup' ) ) {
	add_action( 'planmyday_action_before_init_theme', 'planmyday_template_header_4_theme_setup', 1 );
	function planmyday_template_header_4_theme_setup() {
		planmyday_add_template(array(
			'layout' => 'header_4',
			'mode'   => 'header',
			'title'  => esc_html__('Header 4', 'planmyday'),
			'icon'   => planmyday_get_file_url('templates/headers/images/4.jpg')
			));
	}
}

// Template output
if ( !function_exists( 'planmyday_template_header_4_output' ) ) {
	function planmyday_template_header_4_output($post_options, $post_data) {
		...
		<header class="top_panel_wrap top_panel_style_4 scheme_<?php echo esc_attr($post_options['scheme']); ?>">
			<div class="top_panel_wrap_inner top_panel_inner_style_4 top_panel_position_<?php echo esc_attr(planmyday_get_custom_option('top_panel_position')); ?>">
