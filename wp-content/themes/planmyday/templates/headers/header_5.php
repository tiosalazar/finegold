...
if ( !function_exists( 'planmyday_template_header_5_theme_setup' ) ) {
	add_action( 'planmyday_action_before_init_theme', 'planmyday_template_header_5_theme_setup', 1 );
	function planmyday_template_header_5_theme_setup() {
		planmyday_add_template(array(
			'layout' => 'header_5',
			'mode'   => 'header',
			'title'  => esc_html__('Header 5', 'planmyday'),
			'icon'   => planmyday_get_file_url('templates/headers/images/5.jpg')
			));
	}
}

// Template output
if ( !function_exists( 'planmyday_template_header_5_output' ) ) {
	function planmyday_template_header_5_output($post_options, $post_data) {
		...
		<header class="top_panel_wrap top_panel_style_5 scheme_<?php echo esc_attr($post_options['scheme']); ?>">
			<div class="top_panel_wrap_inner top_panel_inner_style_5 top_panel_position_<?php echo esc_attr(planmyday_get_custom_option('top_panel_position')); ?>">
			
			<?php if (planmyday_get_custom_option('show_top_panel_top')=='yes') { ?>
