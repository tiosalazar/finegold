...
if ( !function_exists( 'planmyday_template_list_theme_setup' ) ) {
	add_action( 'planmyday_action_before_init_theme', 'planmyday_template_list_theme_setup', 1 );
	function planmyday_template_list_theme_setup() {
		planmyday_add_template(array(
			'layout' => 'list',
			'mode'   => 'blogger',
			'need_columns' => true,
			'title'  => esc_html__('Blogger layout: List', 'planmyday')
			));
	}
}

// Template output
if ( !function_exists( 'planmyday_template_list_output' ) ) {
	function planmyday_template_list_output($post_options, $post_data) {
